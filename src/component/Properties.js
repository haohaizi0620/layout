...
class Properties extends Component {
    ...
    updateStateVal() {
        ...
        if (tempKeyVal == 2) {
            ...
            this.setState({
                chart: tempChart.concat(
                    JSON.parse(JSON.stringify(tempLayer))
                  ),
                  tempLayerType:tempLayer
            },() => {
...
class EditSimpleMainInfo extends Component {
    ...
    render() {
        ...
        if(this.props.includeSelectFlag){//当前的childer的
            defaultOneColVal = 0;
            defaultTwoColVal = 24;
        }
        return (
                    ...
                    <Col span={defaultTwoColVal}>
                        ...
                        {this.props.childer.map((item, i) => {
                            ...
                            if (itemType == "InputNumber") {
                                ...
                                return (
                                    <div className="pro-item-simple"> 
                                        <pre  style={{maxHeight:'200px',outline: '1px solid white'}} ><code id="json">{JSON.stringify(item.value, null, '  ')}</code></pre>
                                    </div>
                                )
                            }else if(itemType == "ReactJson"){
                                return (
                                    <div className="pro-item-simple"> 
                                       <ReactJson src={ item.value}  onEdit={event => {
